...
def find_normalizer(source=None):
    ...
    ### To satisfy domainbed datasets
    if isinstance(source, data.dataset.ConcatDataset):
        return find_normalizer(source.datasets[0])
    if hasattr(source, 'underlying_dataset'):
        print("checking underlying_dataset")
        return find_normalizer(source.underlying_dataset)
