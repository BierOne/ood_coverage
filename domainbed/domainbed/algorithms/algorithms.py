...
class ERM_NAC(ERM):
    """
    Empirical Risk Minimization (ERM) with Neuron Activation Coverage
    """
    def __init__(self, input_shape, num_classes, num_domains, hparams, kernel=None):
        super(ERM_NAC, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.kernel = kernel
        self.lamda = 1e-4
    ...
    def update(self, x, y, **kwargs):
        ...
        nac_loss = torch.zeros(1)
        if self.kernel is not None:
            nac_loss = self.kernel.calculate_entropy_loss(feat)
            # print(nac_loss)
            loss = self.lamda * nac_loss

        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        return {'loss': loss.item(), 'nac_loss':nac_loss.item()}
