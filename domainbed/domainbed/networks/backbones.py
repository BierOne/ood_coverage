...
def get_backbone(name, preserve_readout, pretrained):
    ...
    if name == "resnet18":
        ...
        network = timm.create_model("vit_tiny_patch16_224", pretrained=pretrained)
        n_outputs = network.num_features

    elif name == "vit-s16":
        network = timm.create_model("vit_small_patch16_224", pretrained=pretrained)
        n_outputs = network.num_features

    elif name == "vit-b16":
        network = timm.create_model("vit_base_patch16_224", pretrained=pretrained)
        n_outputs = network.num_features

    elif name == "vit-b32":
        network = timm.create_model("vit_base_patch32_224", pretrained=pretrained)
        n_outputs = network.num_features
