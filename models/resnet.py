"""
Source: https://github.com/deeplearning-wisc/react/blob/master/models/resnet.py#L442
"""
...
class ResNet(AbstractResNet):
    ...
    def forward(self, x):
        if self.use_thresh:
            return self.forward_threshold(x, self.thresh)
        x = self.features(x)
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        x = self.fc(x)
